...
{
    private Dictionary<int, Dictionary<MessageType, Queue<byte[]>>> LastMessageBroadcastToClients = new();
    private Dictionary<int, Dictionary<MessageType, float>> resendPackageCounterToClients = new();
            ...
            foreach (int id in resendPackageCounterToClients.Keys)
            {
                foreach (MessageType messageType in resendPackageCounterToClients[id].Keys)
                {
                    ...
                    if (resendPackageCounterToClients[id][messageType] >= ((ServerPingPong)pingPong).GetLatencyFormClient(id) * 5)
                    {
                        ...
                        {
                           UnityEngine.Debug.Log("Resending package to client " + id);
